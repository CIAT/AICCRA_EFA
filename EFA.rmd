...
StartYear<-reactive({as.numeric(input$StartYear)})
```
:::
:::{}
...
Create NPV function
```{r Analysis - NPV function, echo=T}
...
Stats.Spill.Disp<-reactive({
  FormatExtraction(Data=data.table::copy(CoreAreasSpill()),
                 vop_cols=vop_cols(),
                 area_cols=area_cols(),
                 prod_cols=prod_cols(),
                 USD_Units=as.numeric(input$USD_Units),
...
Explanation of Investment tab headings: 
**`Ad.Rate`** = annual adoption rate (proportion); 
**`Pr.Inc`** = productivity increment (proportion); 
