...
knitr::opts_knit$set(root.dir = "C:/Documents/OneDrive - CGIAR/Projects/AICCRA/AICCRA_EFA")
...
renderPlot({terra::plot(log(LSvop()),
                        fun=AddBorders(),
                        pax=list(cex.axis=input$TextSize),
                        plg=list(x=input$LegPos,cex = input$TextSize,ncol=input$LegCols),
                        col=viridis(n=20,
...
LSprod2<-reactive({
  terra::mask(terra::crop(
    terra::rast(paste0("Data/Herrero/Resampled/",LSvop_options[System %in% input$LSChoice & Variable=="MeatYield",Filename]))*CellSize.km/1000
  ,Farming_System()),Farming_System())
})
...
**Livestock production ln(t/pixel)**
...
**MapSPAM Production - ln(ton/pixel)**
...
:::
:::{}
*Benefit-cost ratio for non-adopters*  
*Harris and Orr (2013) find this is *  
*1.62 for "traditional" technologies*
```{r Parameter - Non BCR,echo=F}
  numericInput(inputId="Non_BCR", 
              label="BCR of non-adopters",
              value = 1.62,
              min=0,              
              max=10,
              step = 0.01,
...
prod_cols<-reactive({c(names(MSprod()),names(LSprod()))})
area_cols<-reactive({c(names(MSarea()),names(LSarea()))})
vop_cols<-reactive({c(names(MSvop()),names(LSvop()))})
```
...
```{r Analysis - NPV & BCR wrapper,echo=F}
InvestDataWrapper<-function(Data,InvestStatCrops,ProjectCost){
  InvestData1<-Data[Crop %in% InvestStatCrops, list(Farming_System,Country,Crop,Ad.Rate, Pr.Inc, Project_benefits, VOP_CIS, Year, Total_adoption, AvoidedLossPerc)
                      ...
                      ][,PB_disc:=Project_benefits/((1+DiscountRate())^Year)
                        ][,PB_CIS_disc:=Project_benefits_CIS/((1+DiscountRate())^Year)
                          ][,VOP_CIS:=NULL]


InvestData<-melt(InvestData1,
     id.vars = c("Farming_System","Country","Crop","Ad.Rate", "Pr.Inc", "Year", "Total_adoption", "AvoidedLossPerc", "Project_benefits", "Project_benefits_CIS"),
     variable.name = "CISinc",
     value.name = "Cashflow_disc")[,Cashflow:=Project_benefits
                         ][,Project_benefits:=NULL
                           ][CISinc=="PB_CIS_disc",Cashflow:=Project_benefits_CIS
                             ][,Project_benefits_CIS:=NULL
                               ][CISinc=="PB_CIS_disc",CISinc:="Yes"
                                 ][CISinc=="PB_disc",CISinc:="No"]
  
  NPV<-  InvestData[,list(Avoided_loss_mean=-mean(AvoidedLossPerc,na.rm=T)/100,
                      Value=sum(Cashflow_disc,na.rm=T),
                      Years=max(Year),
                      Implied_Adoption=max(Total_adoption)
                      ),by=list(Ad.Rate,Pr.Inc,CISinc)
             ][,NPV:=Value-ProjectCost
               ][,BCR:=NPV/ProjectCost
                 ][,Value:=NULL]
  
  return(list(InvestData1=InvestData1,InvestData=InvestData,NPV=NPV))
}
```
...
```{r Analysis - MI Core DT, echo=F}
  renderDT({
    datatable(dplyr::mutate_if(data.table::copy(MI.Core())[,Ad.Rate:=as.character(Ad.Rate)],is.numeric,~round(.,3)),
                caption="",
                extensions = 'Buttons',
                filter = "top",
                options = list(dom = 'Blfrtip',
...
MI.Core.Total<-reactive({
  ...
  renderDT({
    datatable(dplyr::mutate_if(data.table::copy(MI.Core.Total())[,Ad.Rate:=as.character(Ad.Rate)],is.numeric,~round(.,3)),
                caption="",
                extensions = 'Buttons',
                filter = "top",
                options = list(dom = 'Blfrtip',
...
```{r Analysis - MI Spillover DT, echo=F}
  renderDT({
    datatable(dplyr::mutate_if(data.table::copy(MI.Spillover())[,Ad.Rate:=as.character(Ad.Rate)],is.numeric,~round(.,3)),
                caption="",
                extensions = 'Buttons',
                filter = "top",
                options = list(dom = 'Blfrtip',
...
MI.Spill.Total<-reactive({
  ...
  renderDT({
    datatable(dplyr::mutate_if(data.table::copy(MI.Spill.Total())[,Ad.Rate:=as.character(Ad.Rate)],is.numeric,~round(.,3)),
                caption="",
                extensions = 'Buttons',
                filter = "top",
                options = list(dom = 'Blfrtip',
...
**`Cost_adopt`** = estimate of production cost per ha as `Price*Y_adopt/BCR of non-adopters`;
**`Cost_non_adopt`** = estimate of production cost per ha as `Price*Y_non_adopt/BCR of non-adopters`;
**`Total_adoption`** = total adoption of technology for given `Year` (proportion); 
**`A_non_adopt`** = area of non-adopters as `Area*(1-Total_adoption)` (km2); 
**`A_adopt`** = area of adopters (km2) as `Area*Total_adoption`; 
**`VOP_non_adopt`** = total value of production for non-adopters (international USD) as `A_non_adopt*Y_non_adopt*Price`; **`VOP_adopt`** = total value of production for adopters (international USD) as `A_adopt*Y_adopt*Price`;
**`NR_non_adopt`** = net return for non-adopters as `A_adopt*(Y_adopt*Price-Cost_adopt)`;
...
SpillCVSummary<-reactive({
  SpilloverCV()$Stats[Year==max(Year),
                      list(Area.ha=sum(Area.ha,na.rm=T),
                        ...
                        VOP_non_adopt=sum(VOP_non_adopt,na.rm=T)/as.numeric(input$USD_Units),
                        VOP_adopt=sum(VOP_adopt,na.rm=T)/as.numeric(input$USD_Units),
                        VOP=sum(VOP,na.rm=T)/as.numeric(input$USD_Units),
                        VOP_CIS=sum(VOP_CIS,na.rm = T)/as.numeric(input$USD_Units),
                        VOP_Both=sum(VOP_Both,na.rm = T)/as.numeric(input$USD_Units)), 
                      by=list(Crop,Ad.Rate,Pr.Inc)
                  ][,Ad.Rate:=as.character(Ad.Rate)][,VOP_Total:=VOP_non_adopt+VOP_adopt]
...
**`Cashflow_disc`** discounted marginal value of adoption (international USD) as `Cashflow/(1+DiscountRate)^Year`; 
...
```{r Analysis - NPV Calc 1 DT Core, echo=F}
  ...
  renderDT({
    datatable(dplyr::mutate_if(InvestDataCore()$InvestData[,Cashflow_disc:=Cashflow_disc/as.numeric(input$USD_Units)
                                               ][,Cashflow:=Cashflow/as.numeric(input$USD_Units)],is.numeric,~round(.,3)),
                caption="",
                extensions = 'Buttons',
                filter = "top",
                options = list(dom = 'Blfrtip',
...
InvestDataCS<-reactive({
  InvestDataWrapper(Data= rbind(CoreCV()$Stats,SpilloverCV()$Stats),
                    InvestStatCrops=input$InvestStatCrops,
                    ProjectCost=ProjectCost())
})
```
...
```{r Analysis - NPV Calc 1 DT, echo=F}
  ...
  renderDT({
    datatable(dplyr::mutate_if(InvestDataCS()$InvestData[,Cashflow_disc:=Cashflow_disc/as.numeric(input$USD_Units)
                                               ][,Cashflow:=Cashflow/as.numeric(input$USD_Units)],is.numeric,~round(.,3)),
                caption="",
                extensions = 'Buttons',
                filter = "top",
...
Each row in the table presents NPV, BCR and IRR investment indicators for each combination of annual adoption rate and productivity increment. 
Explanation of table headings: 
**`Ad.Rate`** = annual adoption rate (proportion); 
**`Pr.Inc`** = productivity increment (proportion); 
**`CISinc`**  indicates if estimates of CIS benefits are included in calculations or not; 
**`Avoided_loss_mean`** = the mean reduction in yield losses due to climate variability from use of CIS (proportion), the mean is calculated across all farming system x crop x country combinations; 
**`Years`** = Project duration; 
**`Implied Adoption`** = total assumed adoption of technologies in core countries; 
**`NPV`** net-present value (international USD); 
**`BCR`** = benefit-cost ratio as `NPV/ProjectCost`; 
**`IRR`** = internal rate of return (see the `irr` function in **Analysis::Data prep & analysis functions** tab for methodological details).
...
###### Core only
*Investment indicators calculated only considering core countries*
```{r Analysis - IRR Calc Core,echo=F}

InvestStats_IRR<-reactive({
  irr_wrapper(NPV=InvestDataCore()$NPV,
              InvestData=InvestDataCore()$InvestData,
              ProjectCost=ProjectCost())
})
```
...
```{r Analysis - IRR Calc DT Core, echo=F}
  renderDT({
    datatable(data.table::copy(InvestStats_IRR())[,NPV:=round(NPV/as.numeric(input$USD_Units),3)
                                                  ][,BCR:=round(BCR,3)
                                                  ][,IRR:=round(IRR,3)
...
```{r Analysis - IRR Calc Spillover,echo=F}
...
```{r Analysis - IRR Calc DT Spillover, echo=F}
  renderDT({
    datatable(data.table::copy(InvestStats_IRRspill())[,NPV:=round(NPV/as.numeric(input$USD_Units),3)
                                                  ][,BCR:=round(BCR,3)
                                                  ][,IRR:=round(IRR,3)
                                                    ][,Implied_Adoption:=round(Implied_Adoption,3)
                                                      ][,Avoided_loss_mean:=abs(round(Avoided_loss_mean,3))],
