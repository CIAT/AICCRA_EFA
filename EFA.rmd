...
MarginalInc<-function(Ad.Rate,Pr.Inc,Years,Yield,Area,Price,VOP,CostFactor){
  ...
  Data[,Price:=Price][,Total_Adoption:=cumsum(Total_Adoption)
               ...
               #][,Marginal_VOP:=VOP_adopt*(Y_adopt-Y_non_adopt)/CostFactor
                ][,Marginal_VOP:=A_adopt*Price*(Y_adopt-Y_non_adopt)/CostFactor
...
MIwrapper<-function(Data,Ad.Rates,Pr.Inc,Years,Commodity,CostFactor){
...
AvLossWrapper<-function(CV_Data,Stats,CV_Change,CostFactor){
  ...
  Stats[,AvoidedLossPerc:=CV_Data[N,AvoidedLossPerc]
              ...
              ][is.na(Marginal_VOP_Both),Marginal_VOP_Both:=Marginal_VOP]
...
MI.Spill.Total<-reactive({
  Stats.Spillover()[Year==max(Year),list(Area.ha=sum(Area.ha,na.rm=T),
                  A_non_adopt=sum(A_non_adopt,na.rm=T),
                  A_adopt=sum(A_adopt,na.rm=T),
                  VOP_non=sum(VOP_non,na.rm=T)/as.numeric(input$USD_Units),
                  VOP_adopt=sum(VOP_adopt,na.rm=T)/as.numeric(input$USD_Units),
...
CoreCVSummary<-reactive({
  CoreCV()$Stats[Year==max(Year),
                 list(Area.ha=sum(Area.ha,na.rm=T),
                    ...
                    Marginal_VOP_Both=sum(Marginal_VOP_Both,na.rm = T)/as.numeric(input$USD_Units)),
...
```{r Analysis - IRR Calc DT Core, echo=F}
  renderDT({
    datatable(data.table::copy(InvestStats_IRR())[,NPV:=NPV/as.numeric(input$USD_Units)
                                                  ][,BCR:=round(BCR,3)
                                                  ][,IRR=round(IRR,3)
...
```{r Analysis - IRR Calc DT Spillover, echo=F}
  renderDT({
    datatable(data.table::copy(InvestStats_IRRspill())[,NPV:=NPV/as.numeric(input$USD_Units)
                                                  ][,BCR:=round(BCR,3)
                                                    ][,IRR=round(IRR,3)
                                                      ][,Implied_Adoption:=round(Implied_Adoption,3)],
                caption="",
